...
'''
Last script for the threshold analysis. U can run it locally. The script sums over all files with the same passband if plot_only= False.
If you just want to plot the results in directory results choose plot_only= True. 
'''

parser = argparse.ArgumentParser(description='Noise Trigger Rate')
parser.add_argument('passband_low', type=int, nargs='?', default = 80, help = 'lower bound of passband in input file name, most likely in MHz')
parser.add_argument('passband_high', type=int, nargs='?', default = 180, help = 'higer bound of passband in input file name, most likely in MHz')
parser.add_argument('number_of_files', type=int, nargs='?', default = 1, help = 'number of input files to loop over, most likely last number in filename')
parser.add_argument('plot_only', type=bool, nargs='?', default = False, help = 'choose if u want to plot the results of one dic (then True) otherwise it will combine all dict with same passband')
...
parser.add_argument('input_filename', type=str, nargs='?', default = 'output_threshold_final/final_threshold_pb_{:.0f}_{:.0f}_i5_{}.pickle'.format(
        passband_low, passband_high, number_of_files), help = 'input filename from script 2_theshold_final.py, see default for formating')
