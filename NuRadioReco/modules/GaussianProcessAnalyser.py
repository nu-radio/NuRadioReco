...
import logging
logger = logging.getLogger('GaussianProcessAnalyzer')
logging.basicConfig()
...
class GaussianProcessAnalyser:
    ...
    """
    Module which filters out noise on the trace for a given station and events using
    Locally Coupled Gaussian Process Regression
    Code for the Gaussian Process Analysis (local_GP_toolbox.py and example "Local GP toolbox")
    by Luca Ambrogioni: https://github.com/LucaAmbrogioni/LocallyCoupledGP
    Adapted by Debora Dorn (2019)
    ...
    def run(self, station, trace_window = 160*units.nanosecond,
            window_spacing = 5.0*units.nanosecond, window_width = 7.0 *units.nanosecond,
            frequency_min = 30*units.MHz, frequency_max = 300*units.MHz,
            frequency_width = 10.0 *units.nanosecond, signal_amplitude = 0.08*units.V,
            noise_sd = 20* units.mV, signal_cut = False,
            keep_trace_length = True, cut_off_noise = False ):
        """
        Run Locally Coupled Gaussian Processes

        Parameters
        -----------
        station: station object
            station object corresponding to event(s) to be analysed
        trace_window: float
            time window (with even number of points wrt sampling rate) big enough to
            include the signal pulse but exclude most of the noise for more efficient
            analysis (make sure there is not too much noise included in this window,
            the GP analysis is less likely to work if a large proportion of the training data is just noise),
            default: 160 ns
        window_spacing: float
            variable for locally coupled Gaussian Process, should be about the time
            length of the period of the oscillation with the highest frequency,
            default: 5 ns
        window_width: float
            standard deviation of the Gaussian window function, should be slightly larger than window_spacing,
            default: 7 ns
        frequency_min : float
            minimum frequency expected to occur in the signal,
            default: 30 MHz
        frequency_max: float
            maximum frequency expected to occur in the signal,
            default: 300 MHz
        frequency_width: float
            this is the range the Gaussian process tries to extrapolate a frequency over,
            should be large enough to "see" the signal is oscillating, but not larger than a
            couple of periods of the highest occuring frequency in the signal,
            default: 10.0 ns
        signal_amplitude: float
            approximate amplitude of the signal,
            default: 0.08 V
        noise_sd: float
            estimated noise level of the signal,
            default: 20 mV
        signal_cut: bool
            whether or not the pulse has already been isolated from the remaining noisy signal
            (using the channelLengthAdjuster module),
            default: False, UNTESTED FEATURE
        keep_trace_length: bool
            if true, makes sure the output trace has the same length as the original trace,
            default: True
            (False:LEADS TO PROBLEM WITH GETTING THE SPECTRUM RIGHT NOW for trace_max=270,
            time and training_data length are fine, ValueError: Dimension n should be a positive
            integer not larger than the shape of the array along the chosen axis,
            also finds division by zero in rfftfreq -> probably in 1/sampling_rate? )
        cut_off_noise: bool
            if true all values of the trace except those in the trace_window are set to zero,
            if false the original values of the trace within the trace window are replaced by
            the noise filtered values and all other values are left as they are,
            default: False, WARNING: gives an error message if set to True when used in
            FullReconstruction
        ...
        try:
            ...
            logger.error("Local GP toolbox not installed, cannot run analysis.")
            sys.exit()

        if (signal_cut == False) or (keep_trace_length == True) or (cut_off_noise = False):
            logger.warning("Using untested configurations. May lead to errors.")
        ...
        for channel in station.iter_channels():
            ...
            if signal_cut:
                ...
                if trace_max < trace_window*sampling_rate/2:
                    ...
                    cut_left = True
                    logger.info("Couldn't split the window equally on either side of the pulse maximum. Some points to the left of the pulse were cut off. ")
                ...
                #window doesn't have enough data points to the right of the signal maximum
                    ...
                    training_data = trace[trace_max-int(np.round(trace_window*sampling_rate/2)):][::-1]
                    cut_right = True
                    logger.info("Couldn't split the window equally on either side of the pulse maximum. Some points to the right of the pulse were cut off. ")
                ...
                #window doesn't have enough data points to the left and right of the signal maximum
                    ...
                    training_data = trace[::-1]
                    cut_left_right = True
                    logger.info("The time window was larger than the trace, adjusted it accordingly.")
                    ...
                    logger.info('max', trace_max)
                    logger.info('time', np.arange(time_min, time_max, 1/sampling_rate).size)
                    logger.info('data', training_data.size)
                    logger.info('Succesfully cut off the trace on either side of the pulse.')
                ...
                if time_max - time_min != trace_window:
                    ...
                    logger.info('Fixed time_min and time_max')
