...
class voltageToAnalyticEfieldConverterNeutrinos:
	...
	This module is specified for neutrinos. The function used to describe the electric field is the Alvarez2009 parametrization. 
	...
	def run(self, evt, station, det, icemodel, shower_type = 'HAD', debug= True, channel_Vpol = 0, channel_Hpol = 1, channels_phasing_Vpol = [2,4,5],channels_phasing_Hpol = [3], phasing =False, passband = [200* units.MHz, 500 * units.MHz], bandpass = True, use_MC=True, attenuation = True,  attenuation_model = 'GL1', sampling_rate = 5*units.GHz, parametrization = 'Alvarez2009'):
		...
		passband: [float, float]
			the lower and upper frequency for which the analytic pulse is calculated and the fit is performed
		bandpass: bool
		...
		parametrization:
			parametrization model used for the reconstruction. At the moment only optimized for Alvarez2009
		"""
		...
		phasing_dict = {channel_Vpol: channels_phasing_Vpol + [channel_Vpol], channel_Hpol: channels_phasing_Hpol + [channel_Hpol]}
		...
		dt = 1./sampling_rate
		order = 10
		efield_antenna_factor, V, V_timedomain = get_array_of_channels(station, use_channels_efield, det, zenith, azimuth, antennapattern.AntennaPatternProvider(), time_domain=True) 
		...
		def minimizer_theta_component(params, R, n_index, toffset, minimizer = True, debug = False, hilbert = False):
			N = 50
			if len(params) == 1:
				E_theta = params[0]
				E_phi = E_theta
			...
			time_trace_thet = get_time_trace(E_theta, theta, N_V_time_domain, dt, shower_type, n_index, R,  parametrization)
			...
			for iCh, trace in enumerate(V_timedomain):
				if use_channels_efield[iCh] == channel_Vpol:
					...
					if hilbert: 
						chi2 += np.sum(abs((abs(signal.hilbert(np.roll(analytic_trace, int(toffset))))[k - N: k+2*N] - abs(signal.hilbert(trace))[k - N: k+2*N]))**2)
					else:
						chi2 += np.sum((np.roll(analytic_trace, int(toffset))[k-N:k+3*N] - trace[k-N:k+3*N])**2)
					
			return chi2/(2*noise_RMS**2)

		def minimizer(params, R, n_index, toffset, debug = False, hilbert = False, minimizer = True):
			N = 50
			...
			for sign_tmp in [-1, 1]:
				...
				time_trace_thet = get_time_trace(E_theta, theta, N_V_time_domain, dt, shower_type, n_index, R, parametrization)
				trace_freq_thet = fft.time2freq(time_trace_thet, 1/dt)
				time_trace_phi = sign_tmp*get_time_trace(E_phi, theta, N_V_time_domain, dt, shower_type, n_index, R, parametrization)
				...
				for iCh, trace in enumerate(V_timedomain):
					...
					if use_channels_efield[iCh] == channel_Hpol:
						...
						if hilbert: 
							tmp += np.sum(abs((abs(signal.hilbert(np.roll(analytic_trace, int(toffset))))[k - N: k+N] - abs(signal.hilbert(trace))[k - N: k+N]))**2)
						else:
							tmp += np.sum((abs(np.roll(analytic_trace, int(toffset))[k - N: k+N] - trace[k - N: k+N]))**2)
		...
		def get_phased_trace( channel_id, channel_ids_phasing, band_pass = bandpass):
			...
			for iCh, trace in enumerate(V_timedomain): 
				if use_channels_efield[iCh] in channel_ids_phasing:
					positions = np.array([det.get_relative_position(101,channel_id), det.get_relative_position(101,use_channels_efield[iCh])])
					time_delay1 = get_time_delay_from_direction(zenith, azimuth, positions[0], n = n_index)
					time_delay2 = get_time_delay_from_direction(zenith, azimuth, positions[1], n = n_index)
		...
		r = prop(vertex_position, antenna_position, icemodel, attenuation_model = attenuation_model)
		r.find_solutions()
		...
		time_trace = get_time_trace(10**18,viewing_angle, N_V_time_domain, dt, shower_type, n_index, R, parametrization) 
		...
		analytic_trace_fft = np.sum(efield_antenna_factor[use_channels_efield.index(channel_Vpol)]*np.array([trace_freq, trace_freq]), axis = 0)
		for iCh, trace in enumerate(V_timedomain):
		...
		bnds = ((10**17, 10**20))
		results = opt.minimize(minimizer_theta_component, x0 = (10**18), method = 'Nelder-Mead', args = (R, n_index, toffset), bounds = bnds, options = options) #Fit amplitude theta component with viewing angle fixed to 55 degrees
		print("results", results)

		bnds = ((10**17, 10**19), (np.deg2rad(40), np.deg2rad(70)))
		results = opt.minimize(minimizer_theta_component, x0 = [results.x[0], np.deg2rad(55)], method = 'Nelder-Mead', args = (R, n_index, toffset), bounds = bnds) #Fit amplitude theta component and viewing angle
		print("results ", results)

		
		
		#adapt the time for the fit for the shift of the Hpol wrt Vpol
		positions = np.array([det.get_relative_position(101,channel_Vpol), det.get_relative_position(101,channel_Hpol)])
		time_delay1 = get_time_delay_from_direction(zenith, azimuth, positions[0], n = n_index)
		time_delay2 = get_time_delay_from_direction(zenith, azimuth, positions[1], n = n_index)
		...
		bnds = ((10**17, 10**19))
		results1 = opt.minimize(minimizer, x0=(results.x[0]), method = 'Nelder-Mead', args = (R, n_index, time_shift_Hpol, False, False, True), bounds = bnds, options = options) #Fit amplitude phi component
		print("results", results1)


		
		#store electricfield 
		time_trace_theta = get_time_trace(results.x[0], results.x[1], N_V_time_domain, dt, shower_type, n_index, R, parametrization)
		time_trace_phi = get_time_trace(results1.x[0], results.x[1], N_V_time_domain, dt, shower_type, n_index, R, parametrization)
		...
		voltages_from_efield = trace_utilities.get_channel_voltage_from_efield(station, electric_field, [channel_Vpol, channel_Hpol], det, zenith, azimuth, self.antenna_provider, False)
		...
		for i_trace, v_trace in enumerate(voltages_from_efield):
			...
			time_shift = get_time_delay_from_direction(zenith, azimuth, det.get_relative_position(station.get_id(), [channel_Vpol, channel_Hpol][i_trace])) - (channel.get_trace_start_time() - average_trace_start_time)
		...
		if debug:
			...
			time_trace_thet = get_time_trace(E_theta, view_angle, N_V_time_domain, dt, shower_type, n_index, R, parametrization)
			sign_phi_component = minimizer([E_theta, view_angle, results1.x[0]], R, n_index, toffset, minimizer = False)
			time_trace_phi = sign_phi_component * get_time_trace(E_phi, view_angle, N_V_time_domain, dt, shower_type, n_index, R, parametrization)
			trace_freq_thet = fft.time2freq(time_trace_thet, 1/dt)
			trace_freq_phi = fft.time2freq(time_trace_phi, 1/dt)
			...
			for energy in E:
				for view_angle in theta:
					zplot[i] = np.exp(-1*minimizer_theta_component([energy, view_angle], R, n_index, toffset, minimizer = True))
					xplot[i] = energy
					yplot[i] = view_angle
					i += 1
			
			
			cs = ax2[0].scatter(xplot, np.rad2deg(yplot), c = zplot, cmap = 'gnuplot2', vmin = min(zplot), vmax = max(zplot), s = 100, alpha = .8)
			ax2[0].axhline(np.rad2deg(viewing_angle), color = 'red', label = 'simulated viewing angle', linewidth = 3)
			ax2[0].axhline(np.rad2deg(results.x[1]), color = 'orange', label = 'reconstructed values', linewidth = 3)
			...
			for ip, p in enumerate(polarization):
				xplot.append(p)
				yplot.append(np.exp(-1*minimizer([results.x[0]*np.tan(p)**2], R, n_index, toffset, minimizer = True)))
			...
			ax2[1].axvline(np.rad2deg(np.arctan2(np.sqrt(results1.x[0]), np.sqrt(results.x[0]))), label = 'reconstructed polarization', color = 'orange', linewidth = 3)
